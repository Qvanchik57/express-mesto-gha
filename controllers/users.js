...
module.exports.patchProfile = async (req, res, next) => {
  ...
  await Users.findByIdAndUpdate(
    ...
    .catch((err) => {
      if (err.name === 'ValidationError') {
        next(new ValidationError({ message: 'Переданы некорректные данные при обновлении профиля' }));
      } else {
        next(new DefaultError({ message: 'Ошибка по умолчанию' }));
      }
    });
...
module.exports.patchAvatar = async (req, res, next) => {
  ...
  await Users.findByIdAndUpdate(
    ...
    .catch((err) => {
      if (err.name === 'ValidationError') {
        next(new ValidationError({ message: 'Переданы некорректные данные при обновлении аватара' }));
      } else {
        next(new DefaultError({ message: 'Ошибка по умолчанию' }));
      }
    });
